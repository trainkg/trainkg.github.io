...
> 渲染函数是组件的核心接口，最大灵活性控制组件的生命周期，一般核心概念围绕render,re-render, 比如较早的underscore

**渲染函数范例 - 常规模板， 在script中定义组件模板**
...
```javascript
// 标记组件 anchored-heading 使用对应的模板定义
...
上面是一个组件的最基础的样子，也是以前比较老的前端渲染框架都支持的方式， （模板内容不同，取决与模板引擎的解析能力和特性）
...
```javascript
Vue.component('anchored-heading', {
  // 核心关键createElement方法，提供模板编译功能. 
  // 类似Underscore里面的template方法，VUE功能更强
...
![官方图片](https://cn.vuejs.org/images/dom-tree.png)

```html
// 常规占位, 只不过vue严格规定了data,props. (属性做了划分，就会有对应的数据特性)
<h1>{{ blogTitle }}</h1>
```
...
Vue 通过建立一个**虚拟 DOM** 来追踪自己, 要如何改变真实 DOM
...
> 观念： 在VUE自己的空间中管理所有的节点，不是通过原生的document. 掌握了对象，就掌握了他的一切 （参考spring , 吐槽一下angular, 虽然我不是专业的前端人员，但是也是使用了很久， 前端应该把核心放在渲染上面）

#### 2.2 createElement 参数
...
### [3. 使用 JavaScript 代替模板功能](https://cn.vuejs.org/v2/guide/render-function.html#使用-JavaScript-代替模板功能)

#### 3.1 替换 [`v-if` 和 `v-for`](https://cn.vuejs.org/v2/guide/render-function.html#v-if-和-v-for)
...
#### [3.2 `v-model`](https://cn.vuejs.org/v2/guide/render-function.html#v-model)
...
#### 3.3 [事件 & 按键修饰符](https://cn.vuejs.org/v2/guide/render-function.html#事件-amp-按键修饰符)
...
#### 3.4 [插槽](https://cn.vuejs.org/v2/guide/render-function.html#插槽)
...
render: function (createElement) {
  // `<div><slot></slot></div>`
  return createElement('div', this.$slots.default) //静态插槽
}
```
...
render: function (createElement) {
  ...
  return createElement('div', [
    this.$scopedSlots.default({ 	//作用域插槽
      text: this.message
    })
  ])
}
...
### 4. [JSX](https://cn.vuejs.org/v2/guide/render-function.html#JSX)
...
### 5. [函数式组件](https://cn.vuejs.org/v2/guide/render-function.html#函数式组件)
...
#### 5.1 [向子元素或子组件传递 attribute 和事件](https://cn.vuejs.org/v2/guide/render-function.html#向子元素或子组件传递-attribute-和事件)
...
#### 5.2 [`slots()` 和 `children` 对比](https://cn.vuejs.org/v2/guide/render-function.html#slots-和-children-对比)

> children没有主次概念， slot强调主次
...
[组件]: https://www.jianshu.com/p/3ae16fba1fe9	"动态组件"
