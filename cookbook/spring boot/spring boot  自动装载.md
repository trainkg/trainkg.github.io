## Spring boot 自动装配 [需要详细更新]

[TOC]

[官方文档URL](https://docs.spring.io/spring-boot/docs/current/reference/html/using-spring-boot.html#using-boot-locating-the-main-class)
...
### class的配置方式
  ...
  从官方描述来看，还是推荐使用`java`注解的方式定义配置

  > 不过注解方式定义配置有一定的局限性， 修改配置需要重新进行文件编译， 这一点应该需要得到衡量。
  >
  > 所以注解方式的配置为了灵活性建议结合配置文件一起使用
...
当然官方还是强烈你别不要这么干， 还是推荐使用@configuration
  @ImportResource
...
####  默认的自动装配信息
 ...
 查看`spring-boot-autoconfigure/META-INF/spring.facotories`

 使用spring security 的自动装配配置作为示例.   
